class essentials expands Mutator config(dropall);


var bool serverabrupt;
var int achevmentcount;        // decrare our avalible achevment count!
...
var object_completer object_completer_ref;
...
function PostBeginPlay()
{
  ...
  if (countm > 1)  {
	...
	log ("-----------------------------------------------------------------------------",stringtoname("------------"));
	log (" Loading Essentials v1b for 227i",stringtoname("[Essentials]"));
	log ("-----------------------------------------------------------------------------",stringtoname("------------"));
	...
	MakeEndingsVisible();
	spawnmutators();
	object_completer_ref = Spawn(class'object_completer');
	//AppSeconds(appsex2);
 ...
 { // first argument is one teleport statment
  ...
  return;
	...
	//if ( InStr(ClassName,".")==-1 )
		...
		if ( InStr(ClassName,".")==-1 )
		{
        templ = returnmatchingfile(Classname)$"." $ ClassName;
    ...
    {
	tempp.ClientMessage(txtclr()$"Summon:  No such package/playerid ("$ ReplaceStr(caps(classname), caps("broken."), "")$ ") or error.");
	}
...
{
local string test;
if (object_completer_ref != none)
{
test= object_completer_ref.returnmatchingfile(objectname);
return test;
}else{
...
// this will timeout the users on the server due to the for loop
function testdumpresources()
{
local int z;
if (object_completer_ref != none)
{
  For( z = 0; z < Array_Size(object_completer_ref.avalibleobjectsarray) ; z++ )
        {
        log(object_completer_ref.avalibleobjectsarray[z].objpack $ "." $ object_completer_ref.avalibleobjectsarray[z].objclass);
        }
}else{
log("error: object_completer unavalible",'dumpresourse');
}
